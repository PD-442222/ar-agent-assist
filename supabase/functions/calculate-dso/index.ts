...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (outstandingAmountTotal > 0 && weightedOutstandingDays > 0) {
      ...
      console.log(
        `DSO calculated from outstanding invoices: ${avgDSO} days across ${outstandingInvoices?.length || 0} invoices`,
      );

      return new Response(JSON.stringify({ dso: avgDSO }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 200,
      });
    }
    ...
    if (totalPaidAmount > 0 && weightedPaymentDays > 0) {
      ...
      console.log(
        `DSO calculated from matched payments: ${avgDSO} days across ${matchedPayments?.length || 0} payments`,
      );
    ...
    return new Response(JSON.stringify({ dso: fallbackDSO }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 200,
    });
  } catch (error: any) {
